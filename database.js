...
export async function saveMenuItems(menuItems) {
  return new Promise((resolve) => {
    db.transaction((tx) => {
      menuItems.map((item) => {
        const query = `insert into menuitems (uuid, title, price, category) values(${item.id}, "${item.title}", ${item.price}, "${item.category}");`
...
export async function clearItems(menuItems) {
  return new Promise((resolve) => {
    db.transaction((tx) => {
      const query = `delete from menuitems;`
      tx.executeSql(query, [], (_, { rows }) => {
        resolve(rows._array);
      });
    });
...
export async function filterByQueryAndCategories(query, activeCategories) {
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      const queryItem = `select * from menuitems where title like '%${query}%' and category in ("${activeCategories.join('", "')}")`
      tx.executeSql(queryItem, [], (_, { rows }) => {
        resolve(rows._array);
